...
long pmod[] = {4194301, 4194301, 1048573,
                                 524287, 262139, 131071,
                                 65521, 32749} ;
long mult[] = {914334, 360889, 380985,
            283741, 92717, 43165,
            17364, 219}; 
...
#pragma omp parallel firstprivate(pmod, mult)
{
    int id = omp_get_thread_num();
    Random rand (mult[id], pmod[id]);
    rand.seed(-r, r); // The circle and square are centered at the origin
    printf("id:%d, mult=%li, pmod=%li\n", id, mult[id], pmod[id]);

    #pragma omp for reduction(+ : Ncirc) private(test, x, y) 
